...
from models.layers.event_filter import EventFilterModule
...
class RNNDetector(BaseDetector):
    def __init__(self, mdl_config: DictConfig):
        ...
        enable_event_filtering = mdl_config.get('enable_event_filtering', False)
        ...
        # Add event filtering module
        self.enable_event_filtering = enable_event_filtering
        if self.enable_event_filtering:
            self.event_filter = EventFilterModule(input_channels=in_channels)
            print("Initialized Event Signal Filtering Module")
    ...
    def forward(self, x: torch.Tensor, prev_states: Optional[LstmStates] = None, token_mask: Optional[torch.Tensor] = None) \
        ...
        for stage_idx, stage in enumerate(self.stages):
            x, state, p = stage(x, prev_states[stage_idx], token_mask if stage_idx == 0 else None, r[:, stage_idx], stage_idx)
            states.append(state)
...
class SASTAttentionPairCl(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos_emb: nn.Module, r: torch.Tensor, index_list, stage_idx: int = 0) -> Tuple[torch.Tensor, ...]:
        x, p_loss, index_list = self.att(x, pos_emb, r, index_list, stage_idx)
...
class RNNDetectorStage(nn.Module):
    ...
    def __init__(self,
        ...
        # 不再需要计算stage_id，因为会直接从forward方法传入
        ...
        blocks = [SASTAttentionPairCl(dim=stage_dim,
                                      ...
                                      attention_cfg=attention_cfg, 
                                      first_block=i == 0) for i in range(num_blocks)]  # 不再传递stage_id
    ...
    def forward(self, x: torch.Tensor,
        ...
        for blk in self.att_blocks:
            x, p_loss, r, index_list = blk(x, self.pos_emb, r, index_list, stage_idx)
            P += p_loss
