...
class Module(pl.LightningModule):
    ...
    def training_step(self, batch: Any, batch_idx: int) -> STEP_OUTPUT:
        ...
        #     # 以递归方式搜索模型中的所有自适应阈值学习器
        #     def find_threshold_learners(module, prefix=""):
        #         threshold_learners = []
                
        #         for name, child in module.named_children():
        #             if hasattr(child, 'threshold_learner'):
        #                 threshold_learners.append((f"{prefix}.{name}", child.threshold_learner))
                    
        #             # 递归搜索子模块
        #             child_learners = find_threshold_learners(child, f"{prefix}.{name}")
        #             threshold_learners.extend(child_learners)
                
        #         return threshold_learners
            
        #     # 从模型中查找所有自适应阈值学习器
        #     all_learners = find_threshold_learners(self.mdl.backbone, "backbone")
            
        #     # 打印每个找到的阈值学习器的梯度信息
        #     for path, learner in all_learners:
        #         print(f"\n===== 自适应阈值参数 {path} =====")
        #         print(f"步骤: {step}")
                
        #         # 打印多尺度的基础阈值参数
        #         print("基础阈值(base_bounces):")
        #         for i, bounce in enumerate(learner.base_bounces):
        #             print(f"  stage_{i}: {bounce.item():.6f}")
                
        #         # 打印尺度缩放因子
        #         print("缩放因子(stage_scales):")
        #         for i, scale in enumerate(learner.stage_scales):
        #             print(f"  stage_{i}: {scale.item():.6f}")
                
        #         # 打印温度参数
        #         print("温度参数(temperatures):")
        #         for i, temp in enumerate(learner.temperatures):
        #             print(f"  stage_{i}: {temp.item():.6f}")
                
        #         # 打印偏置参数
        #         print(f"偏置(bias): {learner.bias.item():.6f}")
                
        #         # 打印梯度信息
        #         if learner.base_bounces.grad is not None:
        #             print(f"base_bounces梯度平均值: {learner.base_bounces.grad.abs().mean().item():.6f}")
        #         if learner.stage_scales.grad is not None:
        #             print(f"stage_scales梯度平均值: {learner.stage_scales.grad.abs().mean().item():.6f}")
        #         if learner.temperatures.grad is not None:
        #             print(f"temperatures梯度平均值: {learner.temperatures.grad.abs().mean().item():.6f}")
                
        #         # 打印编码器参数梯度
        #         print("\n编码器梯度信息:")
        #         for name, param in learner.scene_encoder.named_parameters():
        #             if param.grad is not None:
        #                 print(f"  scene_encoder.{name}: grad_mean={param.grad.abs().mean().item():.6f}")
        #         for name, param in learner.density_encoder.named_parameters():
        #             if param.grad is not None:
        #                 print(f"  density_encoder.{name}: grad_mean={param.grad.abs().mean().item():.6f}")
        #         print("===============================")
